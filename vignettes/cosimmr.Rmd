...
```{r}
plot(cosimmr_1_out, type ="beta_histogram", ind = 1)
```
...
boxplots to show change in prop over different covariates?
```{r}
#Look at the change over food source 1 ()
n_samples = length(cosimmr_2_out$output$BUGSoutput$sims.list$p[1,,1])
n_ind = cosimmr_2$n_obs
a = matrix(cosimmr_2_out$output$BUGSoutput$sims.list$p[c(1,10,39,113, 124, 164, 184, 216),,1], ncol = n_samples)
#Extract samples for each group - need to sort by hand I think


Group = c(rep("Group 1", 3600), rep("Group 2", 3600), rep("Group 3", 3600),
          rep("Group 4", 3600), rep("Group 5", 3600), rep("Group 6", 3600),
          rep("Group 7", 3600), rep("Group 8", 3600))

ind1 = cosimmr_2_out$output$BUGSoutput$sims.list$p[1,,1]
ind2 = cosimmr_2_out$output$BUGSoutput$sims.list$p[10,,1]
ind3 = cosimmr_2_out$output$BUGSoutput$sims.list$p[39,,1]
ind4 = cosimmr_2_out$output$BUGSoutput$sims.list$p[113,,1]
ind5 = cosimmr_2_out$output$BUGSoutput$sims.list$p[124,,1]
ind6 = cosimmr_2_out$output$BUGSoutput$sims.list$p[164,,1]
ind7 = cosimmr_2_out$output$BUGSoutput$sims.list$p[184,,1]
ind8 = cosimmr_2_out$output$BUGSoutput$sims.list$p[216,,1]


vec = c(ind1, ind2, ind3, ind4, ind5, ind6, ind7, ind8)


df_plot = data.frame(samples = vec, Group = Group)

library(ggplot2)
ggplot(data = df_plot, aes(x = Group, y = samples, colour = Group))  + geom_boxplot() + ggtitle("Zostera consumed by each group") +xlab("Group") + ylab("Proportion of Zostera consumed") +theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
