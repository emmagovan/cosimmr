...
```{r}
plot(cosimmr_1_out, type ="beta_histogram", obs = 1)
```
...
```{r}
plot(cosimmr_2_out, type = "covariates_plot", source = c("U.lactuca"), cov_name = "Group")
```
...
```{r}
plot(cosimmr_ali_out, type = c("prop_histogram", "beta_histogram"), binwidth = 0.01, obs = c(1,2))
```
...
```{r}
plot(cosimmr_ali_out, type = "covariates_plot", source = "Freshwater")
```
