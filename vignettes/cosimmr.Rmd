...
cosimmr_1_out1 = cosimmr_ffvb(cosimmr_1, error_type = "process+residual")
```
...
plot(cosimmr_1_out1, type ="prop_histogram", obs = 1)
```
...
```{r, eval = FALSE}
cosimmr_ali_out = cosimmr_ffvb(cosimmr_ali, error_type = "process+residual")
cosimmr_ali_out2 = cosimmr_ffvb(cosimmr_ali)
```

We can then look at a summary of the data. This defaults to individual 1
```{r, eval = FALSE}
summary(cosimmr_ali_out, type = "statistics")
summary(cosimmr_ali_out2, type = "statistics")
```
...
```{r, eval = FALSE}
plot(cosimmr_ali_out, type = c("prop_histogram"), obs = c(1), cov_name = "Length")
plot(cosimmr_ali_out2, type = c("prop_histogram"), obs = c(1), cov_name = "Length")
plot(cosimmr_ali_out, type = c("beta_histogram"), obs = c(1), cov_name = "Length")
plot(cosimmr_ali_out2, type = c("beta_histogram"), obs = c(1), cov_name = "Length")
...
alli_pred2 = predict(cosimmr_ali_out2, x_pred)
```
...
summary(alli_pred2, obs = c(1,2,3), type = "statistics")

plot(alli_pred, type = "prop_density")
plot(alli_pred2, type = "prop_density")
```
...
plot(cosimmr_ali_out2, type = "covariates_plot", source = "Freshwater", cov_name = "Length") 
```
