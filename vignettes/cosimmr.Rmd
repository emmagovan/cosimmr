...
summary(cosimmr_2_out, type = "statistics", ind = c(1,10, 110, 114, 125, 165, 185, 220))


Groups3 = matrix(geese_data$groups[1:112])

cosimmr_3 <-cosimmr_load(
    formula = geese_data$mixtures[1:112,] ~ as.factor(Groups3) -1,
    source_names = geese_data$source_names,
    source_means = geese_data$source_means,
    source_sds = geese_data$source_sds,
    correction_means = geese_data$correction_means,
...
summary(cosimmr_3_out, type = "statistics", ind = c(1,10,50))
 #plot(cosimmr_2_out, type = "p_ind")
#library(microbenchmark)

#microbenchmark(cosimmr = cosimmr_ffvb(cosimmr_2), times = 5L)

iteration = cosimmr_2_out$output$lambdares$iteration #So need a way to save this - should be easy to do I think? Just return t?
a = which.max(cosimmr_1_out$output$lambdares$mean_LB[(cosimmr_1_out$output$model$data$ffvb_control$tau +2):iteration])
use_this = cosimmr_1_out$output$lambdares$lambda_save[(cosimmr_1_out$output$ffvb_control$tau +2)+a,]
...
thetares <- sim_thetacpp(n_output, use_this, K, n_tracers, n_covariates, solo)
  ...
  p_sample = array(NA, dim = c(n, n_output, K))
...
summary(cosimmr_2_out, individual = c(1, 10), type = "statistics")#, 10, 115, 125))
...
Running wolves data to see if it agrees with mixsiar
```{r}
wolves_con = read.csv("data/wolves_consumer.csv")
wolves_disc = read.csv("data/wolves_discrimination.csv")
wolves_sources = read.csv("data/wolves_sources.csv")

mixtures = wolves_con[,1:2]
Region = wolves_con$Region
pack = wolves_con$Pack[1:21]
source_names = wolves_sources$X[c(1,4,7)]
source_means = wolves_sources[c(1,4,7),c(3,5)]
source_sds = wolves_sources[c(1,4,7),c(4,6)]
correction_means = wolves_disc[,c(2,4)]
correction_sds = wolves_disc[,c(3,5)]

cosimmr_wolves_p <-cosimmr_load(
    formula = as.matrix(mixtures[1:21,]) ~ as.factor(pack),
    source_names = (source_names),
    source_means = as.matrix(source_means),
    source_sds = as.matrix(source_sds),
   correction_means = as.matrix(correction_means),
    correction_sds = as.matrix(correction_sds)
  )

cosimmr_wolves_r <-cosimmr_load(
    formula = as.matrix(mixtures) ~ as.factor(Region) -1,
    source_names = (source_names),
    source_means = as.matrix(source_means),
    source_sds = as.matrix(source_sds),
   correction_means = as.matrix(correction_means),
...
plot(cosimmr_wolves_p)

wolves_out_p = cosimmr_ffvb(cosimmr_wolves_p)

summary(wolves_out_p, type = "statistics", ind = c(1,10, 21) )


wolves_out_r = cosimmr_ffvb(cosimmr_wolves_r)

summary(wolves_out_r, type = "statistics", ind = c(1,23, 63))


library(simmr)

simmr_p = simmr_load(mixtures = as.matrix(mixtures[1:21,]),
                     source_names = source_names,
                    source_means = as.matrix(source_means),
                    source_sds = as.matrix(source_sds),
                    correction_means = as.matrix(correction_means),
...
out_p = simmr_ffvb(simmr_p)

summary(out_p, group = c(1:3), type = "statistics")

simmr_r = simmr_load(mixtures = as.matrix(mixtures),
                     source_names = source_names,
                    source_means = as.matrix(source_means),
                    source_sds = as.matrix(source_sds),
                    correction_means = as.matrix(correction_means),
...
out_r = simmr_ffvb(simmr_r)

summary(out_r, group = c(1:3), type = "statistics")
```
