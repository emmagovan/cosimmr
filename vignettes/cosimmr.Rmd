...
plot(cosimmr_1_out$output$mean_LB[502:1336])
```
...
```{r}
plot(cosimmr_1_out, type = c("isospace", "beta_histogram", "p_ind"), individuals = c(1,7))
```

More complex examples can also be run. Here we include the group the geese belong to as a covariate
```{r}
data(geese_data)
Groups = matrix(geese_data$groups[1:38])

cosimmr_2 <-cosimmr_load(
    formula = geese_data$mixtures[1:38,] ~ as.factor(Groups),
...
summary(cosimmr_2_out, individual = c(1, 10))


plot(cosimmr_2_out$output$mean_LB[202:2243])
